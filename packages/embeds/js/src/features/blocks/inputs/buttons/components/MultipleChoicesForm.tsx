...
import { ChoiceInputBlock } from '@mozbot.io/schemas'
  ...
  return (
    <form class="flex flex-col items-end gap-2 w-full" onSubmit={handleSubmit}>
      <Show when={props.options?.isSearchable}>
        <div class="flex items-end mozbot-input w-full">
          <SearchInput
            ref={inputRef}
            onInput={filterItems}
            placeholder={
          ...
          {(item) => (
            <span class={'relative' + (isMobile() ? ' w-full' : '')}>
              <div
                ...
                class={
                  'w-full py-2 px-4 font-semibold focus:outline-none cursor-pointer select-none mozbot-selectable' +
          ...
          {(selectedItemId) => (
            <span class={'relative' + (isMobile() ? ' w-full' : '')}>
              <div
                ...
                class={
                  'w-full py-2 px-4 font-semibold focus:outline-none cursor-pointer select-none mozbot-selectable selected'
                }
