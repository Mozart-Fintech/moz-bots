...
import { ChatwootBlock, SessionState } from '@mozbot.io/schemas'
...
const parseChatwootOpenCode = ({
  ...
  resultId,
  mozbotId,
}: ChatwootBlock['options'] & { mozbotId: string; resultId: string }) => {
  const openChatwoot = `${parseSetUserCode(user, resultId)}
  if(window.Mozbot?.unmount) window.Mozbot.unmount();
  window.$chatwoot.setCustomAttributes({
    mozbot_result_url: "${
      env.NEXTAUTH_URL
    }/mozbots/${mozbotId}/results?id=${resultId}",
  });
    ...
    clientSideActions: [
      {
        ...
        chatwoot: {
          scriptToExecute: {
            content: parseVariables(mozbot.variables, { fieldToParse: 'id' })(
              chatwootCode
            ),
            args: extractVariablesFromText(mozbot.variables)(chatwootCode).map(
              (variable) => ({
                id: variable.id,
