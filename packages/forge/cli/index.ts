...
const main = async () => {
  p.intro('Create a new Mozbot integration block')
...
const createTsConfig = async (path: string) => {
  writeFileSync(
    ...
    await prettier.format(
      JSON.stringify({
        extends: '@mozbot.io/tsconfig/base.json',
        include: ['**/*.ts', '**/*.tsx'],
...
import { ${
      ...
      } parseBlockSchema } from '@mozbot.io/forge'
...
async function addBlockToRepoDefinitions(
  ...
  const importStatement = `import { ${camelCaseId}Block } from '@mozbot.io/${id}-block'`
...
async function addBlockToRepoSchemas(
  ...
  const importStatement = `import { ${camelCaseId}Block } from '@mozbot.io/${id}-block'
import { ${camelCaseId}BlockSchema } from '@mozbot.io/${id}-block/schemas'`
...
async function addBlockToCredentialsRepoSchemas(
  ...
  const importStatement = `import { ${camelCaseId}Block } from '@mozbot.io/${id}-block'
import { ${camelCaseId}CredentialsSchema } from '@mozbot.io/${id}-block/schemas'`
...
async function addBlockToRepoPackageJson(id: string) {
  ...
  packageJson.devDependencies[`@mozbot.io/${id}-block`] = 'workspace:*'
  writeFileSync(
