...
import { MozbotPostMessageData } from '../src'
...
describe('createIframe', () => {
  it('should create a valid iframe element', () => {
    ...
    expect(iframeElement.getAttribute('data-id')).toBe(
      'https://mozbot.mozartfintech.com/mozbot-id'
    )
  ...
  it('should parse the right src prop if custom domain and starterVariables', () => {
    ...
    expect(iframes[0].getAttribute('src')).toBe(
      'https://mozbot.mozartfintech.com/mozbot-id?var1=value1&var2=value2'
    )
  })
  ...
  it("shouldn't execute callbacks if event from other than mozbot", async () => {
    expect.assertions(3)
    let n, v
    createIframe({
      url: 'https://mozbot.mozartfintech.com/mozbot-id',
  ...
  it('should close chat when receive close command', async () => {
    ...
    const { open } = Mozbot.initBubble({
      url: 'https://mozbot.mozartfintech.com/mozbot-id2',
    })
    const bubble = document.getElementById('mozbot-bubble')
    open()
    await new Promise((resolve) => setTimeout(resolve, 50))
    expect(bubble?.classList.contains('iframe-opened')).toBe(true)
    const messageData: MozbotPostMessageData = {
