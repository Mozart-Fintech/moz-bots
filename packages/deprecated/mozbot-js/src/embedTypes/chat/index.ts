...
export const initBubble = (params: BubbleParams): BubbleActions => {
  ...
  const existingBubble = document.getElementById('mozbot-bubble') as
    | HTMLDivElement
    | undefined
...
const createBubble = (
  ...
  bubbleElement.id = 'mozbot-bubble'
...
export const getBubbleActions = (
  ...
  const existingBubbleElement =
    bubbleElement ??
    (document.querySelector('#mozbot-bubble') as HTMLDivElement | undefined)
  if (!existingBubbleElement) return { close: () => {}, open: () => {} }
  const existingProactiveMessage =
    proactiveMessageElement ??
    document.querySelector('#mozbot-bubble .proactive-message')
  return {
