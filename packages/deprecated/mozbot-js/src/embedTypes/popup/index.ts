...
export const initPopup = (params: PopupParams): PopupActions => {
  ...
  const existingPopup = document.getElementById('mozbot-popup')
...
const createOverlayElement = (delay: number | undefined) => {
  ...
  overlayElement.id = 'mozbot-popup'
...
export const getPopupActions = (
  ...
  const existingPopupElement =
    popupElement ?? (document.querySelector('#mozbot-popup') as HTMLDivElement)
  return {
    open: () => {
