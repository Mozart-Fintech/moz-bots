...
This video provides a step-by-step guide to successfully configure HTTP request blocks in Mozbot.
...
What I need in my case is instead of inserting "Star Wars", I'd like to insert a Mozbot variable:
...
Feel free to ask the [community](https://mozbot.mozartfintech.com/discord) for help if you struggle setting up a Webhook block.
...
That is expected, Mozbot actually computes all the blocks between each input blocks from a server before displaying anything.
