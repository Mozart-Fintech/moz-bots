...
| NEXTAUTH_URL <span style={{color: '#ef5151'}}>\*</span>           |         | The builder base URL. Should be the publicly accessible URL (i.e. `https://mozbot.domain.com`)                                                                                                                |
...
| NEXT_PUBLIC_DISABLE_SIGNUP                                        | false   | Disable new user sign ups. Invited users are still able to sign up.                                                                                                                                           |
| NEXT_PUBLIC_ONBOARDING_MOZBOT_ID                                  |         | Mozbot ID used for the onboarding. Onboarding page is skipped if not provided.                                                                                                                                |
...
| NEXT_PUBLIC_SMTP_FROM |         | From name and email (i.e. `'Mozbot Notifications' <notifications@host.com>`)                                                                                                                                                                               |
...
Used for authenticating with GitHub. By default, it uses the credentials of a Mozbot-dev app.
...
| S3_BUCKET               | mozbot  | Name of the bucket where assets will be uploaded in.                               |
...
<Note>
  If you'd like to join contribute to Mozbot's translation join the [Discord
...
In Mozbot, Redis is optional and is used to:
...
The [official Mozbot managed service](https://app.mozbot.io/) uses other services such as [Stripe](https://stripe.com/) for processing payments, [Sentry](https://sentry.io/) for tracking bugs and [Sleekplan](https://sleekplan.com/) for user feedbacks.

The related environment variables are listed here but you are probably not interested in these if you self-host Mozbot.
