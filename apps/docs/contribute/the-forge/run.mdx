...
block](https://github.com/baptisteArno/mozbot.io/blob/main/packages/forge/blocks/calCom/actions/bookEvent.ts)
as an example.
...
The `displayEmbedBubble` accepts a `parseInitFunction` function. This function needs to return the same object as the [`web` function](./run#client-function). The function content can use the `mozbotElement` variable to get the DOM element where the block is rendered.
