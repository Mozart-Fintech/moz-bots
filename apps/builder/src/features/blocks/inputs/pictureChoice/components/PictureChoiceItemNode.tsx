...
import { isSvgSrc } from '@mozbot.io/lib'
...
export const PictureChoiceItemNode = ({
  ...
  const { updateItem, createItem, mozbot } = useMozbot()
  ...
  const blockId = mozbot
    ? mozbot.groups.at(indices.groupIndex)?.blocks?.at(indices.blockIndex)?.id
    : undefined
