...
test.describe.parallel('Google sheets integration', () => {
  test('Insert row should work', async ({ page }) => {
    ...
    await importMozbotInDatabase(
      getTestAsset('mozbots/integrations/googleSheets.json'),
      {
        id: mozbotId,
      }
    )
  ...
  test('Get row should work', async ({ page }) => {
    const mozbotId = createId()
    await importMozbotInDatabase(
      getTestAsset('mozbots/integrations/googleSheetsGet.json'),
      {
        id: mozbotId,
      }
    )
    ...
    await page
      ...
      .press('Enter')
    await expect(
      page.locator('mozbot-standard').locator('text=Georges2')
    ).toBeVisible()
  })
})
