...
export const SetVariableSettings = ({ options, onOptionsChange }: Props) => {
  const { mozbot, updateVariable } = useMozbot()
  const selectedVariable = mozbot?.variables.find(
      ...
      <Stack spacing="4">
        ...
        {selectedVariable && !isSessionOnly && !isLinkedToAnswer && (
          <SwitchWithLabel
            ...
            moreInfoContent="By default, the variable is saved only for the user chat session. Check this option if you want to also store the variable in the mozbot Results table."
