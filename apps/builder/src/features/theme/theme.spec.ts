...
test.describe.parallel('Theme page', () => {
  test.describe('General', () => {
    test('should reflect change in real-time', async ({ page }) => {
      ...
      await importMozbotInDatabase(getTestAsset('mozbots/theme.json'), {
        id: mozbotId,
      })
      await page.goto(`/mozbots/${mozbotId}/theme`)
      ...
      await page.click('text="Show Mozbot brand"')
      await expect(page.locator('a:has-text("Made with Mozbot")')).toBeHidden()
      ...
      await expect(page.locator('.mozbot-container')).toHaveCSS(
        'font-family',
        /Roboto Slab/
      )
  ...
  test.describe('Chat', () => {
    test('should reflect change in real-time', async ({ page }) => {
      const mozbotId = 'chat-theme-mozbot'
      try {
        await importMozbotInDatabase(getTestAsset('mozbots/theme.json'), {
          id: mozbotId,
        })
      ...
      // Container
      await expect(page.locator('.mozbot-chat-view')).toHaveCSS(
        'max-width',
        defaultContainerMaxWidth
      )
      await page
      ...
      await expect(page.locator('.mozbot-container img')).toHaveAttribute(
        'src',
        hostAvatarUrl
      )
      await page.click('text=Bot avatar')

      await expect(page.locator('.mozbot-container img')).toBeHidden()
      ...
      const input = page.locator('.mozbot-input')
  ...
  test.describe('Custom CSS', () => {
    test('should reflect change in real-time', async ({ page }) => {
      ...
      await page.fill(
        'div[role="textbox"]',
        '.mozbot-button {background-color: green}'
      )
  ...
  test.describe('Theme templates', () => {
    test('should reflect change in real-time', async ({ page }) => {
      ...
      await page.getByText('Mozbot Dark').click()
