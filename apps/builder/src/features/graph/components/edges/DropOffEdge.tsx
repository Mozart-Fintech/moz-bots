...
export const DropOffEdge = ({
  ...
  const currentBlockId = useMemo(
    () =>
      publishedMozbot?.groups.flatMap((g) => g.blocks)?.find(byId(blockId))?.id,
    [blockId, publishedMozbot?.groups]
  )

  const groupId = publishedMozbot?.groups.find((group) =>
  ...
  const { totalDroppedUser, dropOffRate } = useMemo(() => {
    if (!publishedMozbot || !currentBlockId) return {}
  ...
  }, [currentBlockId, publishedMozbot, totalAnswers, totalVisitedEdges])
  ...
  }, [currentBlockId, publishedMozbot?.groups, sourceEndpoints])
  ...
  const isLastBlock = useMemo(() => {
    if (!publishedMozbot) return false
    const lastBlock = publishedMozbot.groups
      .find((group) =>
        group.blocks.some((block) => block.id === currentBlockId)
      )
      ?.blocks.at(-1)
    return lastBlock?.id === currentBlockId
  }, [publishedMozbot, currentBlockId])
