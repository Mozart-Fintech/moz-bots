...
export const CollaborationList = () => {
  ...
  const handleChangeInvitationCollabType =
    (email: string) => async (type: CollaborationType) => {
      if (!mozbot || !hasFullAccess) return
      const { error } = await updateInvitationQuery(mozbot?.id, email, {
        email,
        mozbotId: mozbot.id,
        type,
      })
  ...
  const handleDeleteInvitation = (email: string) => async () => {
    if (!mozbot || !hasFullAccess) return
    const { error } = await deleteInvitationQuery(mozbot?.id, email)
    if (error)
      return showToast({ title: error.name, description: error.message })
    mutateInvitations({
  ...
  const handleChangeCollaborationType =
    (userId: string) => async (type: CollaborationType) => {
      if (!mozbot || !hasFullAccess) return
      const { error } = await updateCollaboratorQuery(mozbot?.id, userId, {
        userId,
        type,
        mozbotId: mozbot.id,
      })
  ...
  const handleDeleteCollaboration = (userId: string) => async () => {
    if (!mozbot || !hasFullAccess) return
    const { error } = await deleteCollaboratorQuery(mozbot?.id, userId)
    if (error)
      return showToast({ title: error.name, description: error.message })
    mutateCollaborators({
  ...
  const handleInvitationSubmit = async (e: FormEvent) => {
    ...
    const { error } = await sendInvitationQuery(mozbot.id, {
