...
### Overview of Mozbot as a No-Code Chatbot Builder

Mozbot is an open-source platform that makes it easy to create chatbots for React applications. Its key features include:
...
## Method 1: Creating a Simple React Chatbot Using Mozbot

### Setting Up Mozbot Account

1. Visit mozbot.io and sign up for an account.
2. Verify your email address.
3. Log in to the Mozbot dashboard.

### Designing Your Chatbot using Mozbot's Drag-and-Drop Interface

1. Click "Create a new mozbot".
...
### Exporting and Integrating Mozbot Code in React

1. In Mozbot, go to "Share" > "Web".
2. Choose the "React" embed option.
3. Copy the provided code snippet.
4. In your React project, install **Mozbot libraries**:
...
const ChatbotComponent = () => {
  return <Bubble mozbot="YOUR_MOZBOT_ID" />
}

export default ChatbotComponent
```

### Adding Mozbot Chatbot to Existing React Applications
...
5. Use Mozbot's analytics to monitor performance.
6. Iterate on your design based on user interactions and feedback.

These steps show you how to quickly create and integrate a functional chatbot into your React application using **Mozbot's no-code solution**.

<Cta />

## Method 2: Creating the Frontend and Calling the Mozbot API
...
### Setting Up Mozbot API

1. Create a Mozbot account and design your chatbot flow.
2. Generate an API token in Mozbot settings.
3. Note your Mozbot's public ID.
...
### Fetching Responses from Mozbot API
...
const MOZBOT_API_URL = 'https://mozbot.io/api/v1/mozbots'
...
const handleSendMessage = async () => {
  ...
  try {
    let response
    if (!sessionId) {
      response = await axios.post(`${MOZBOT_API_URL}/${PUBLIC_ID}/startChat`, {
        message: inputMessage,
      })
      setSessionId(response.data.sessionId)
    } else {
      response = await axios.post(
        `${MOZBOT_API_URL}/${PUBLIC_ID}/continueChat`,
        {
          sessionId,
          message: inputMessage,
        }
...
By following these steps, you can create a custom **React chatbot frontend** that interacts with the Mozbot API, providing a seamless chat experience for your users.
...
Building a **React chatbot** offers diverse approaches, from no-code solutions to full-stack implementations. This article explored three methods: using Mozbot for rapid deployment, integrating Mozbot's API for customization, and building a complete solution from scratch.
